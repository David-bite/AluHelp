/*PreguntasPDFServlet.java*/
package com.aluhelp.controlador;

import jakarta.servlet.ServletException;
import jakarta.servlet.annotation.WebServlet;
import jakarta.servlet.http.*;

import java.io.File;
import java.io.FileInputStream;
import java.io.IOException;
import java.io.InputStream;

import java.util.*;
import java.util.regex.Matcher;
import java.util.regex.Pattern;
import org.apache.pdfbox.pdmodel.PDDocument;
import org.apache.pdfbox.text.PDFTextStripper;

@WebServlet("/preguntasPDF")
public class PreguntasPDFServlet extends HttpServlet {

    @Override
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {

        HttpSession session = request.getSession();
        Integer usuarioId = (Integer) session.getAttribute("usuarioId");

        if (usuarioId == null) {
            response.sendRedirect("login.jsp");
            return;
        }

        // Recibir el nombre del archivo
        String fileName = request.getParameter("file");

        if (fileName == null) {
            request.setAttribute("error", "No se seleccionó archivo.");
            request.getRequestDispatcher("mis_documentos.jsp").forward(request, response);
            return;
        }

        // GUARDARLO EN SESIÓN (IMPORTANTE)
        session.setAttribute("archivoPDF", fileName);

        // Ruta correcta al PDF
        String pdfPath = getServletContext().getRealPath("/uploads/") + fileName;
        System.out.println("RUTA PDF = " + pdfPath);

        // Leer texto del PDF
        String texto = "";
        try (InputStream input = new FileInputStream(new File(pdfPath)); PDDocument document = PDDocument.load(input)) {

            PDFTextStripper stripper = new PDFTextStripper();
            texto = stripper.getText(document);

        } catch (Exception e) {
            e.printStackTrace();
            texto = "";
        }

        // Generar preguntas
        List<Map<String, Object>> preguntas = generarPreguntasTest(texto);

        // Guardar preguntas en sesión
        session.setAttribute("preguntasPDF", preguntas);

        // Redirigir al examen
        response.sendRedirect("examen?p=0");
    }

private List<Map<String, Object>> generarPreguntasTest(String textoOriginal) {

    List<Map<String, Object>> preguntas = new ArrayList<>();

    if (textoOriginal == null || textoOriginal.length() < 60) {
        return preguntas;
    }

    // 1. Normalizar texto
    String texto = textoOriginal
            .replace("\r", " ")
            .replace("\n", " ")
            .replaceAll("\\s+", " ")
            .trim();

    // 2. Dividir en oraciones (., ?, !)
    String[] oraciones = texto.split("(?<=[\\.\\?\\!])\\s+");
    if (oraciones.length == 0) return preguntas;

    // 3. Construir mapa de frecuencia de palabras (para encontrar conceptos importantes)
    Map<String, Integer> frecuencia = new HashMap<>();

    // Lista simple de stopwords en español
    Set<String> stopwords = new HashSet<>(Arrays.asList(
            "de","la","que","el","en","y","a","los","se","del","las","por","un",
            "para","con","no","una","su","al","lo","como","más","pero","sus",
            "le","ya","o","fue","ha","sí","porque","esta","son","entre","cuando",
            "muy","sin","sobre","también","me","hasta","hay","donde","quien",
            "desde","todo","nos","durante","todos","uno","les","ni","contra",
            "otros","ese","eso","había","antes","él","esto","mi","algunos",
            "qué","unos","yo","otro","otras","otra","tanto","esa","estos",
            "mucho","quienes","nada","muchos","cual","poco","ella","estar",
            "estas","algo","nosotros","ustedes","ellos","ellas","ser","es",
            "era","eran","sea","han","he","has","hay","haya","dicho","dice"
    ));

    // Sacamos palabras del texto para frecuencia
    String textoLower = texto.toLowerCase();
    String[] palabras = textoLower.split("[^a-záéíóúñü0-9]+");

    for (String w : palabras) {
        if (w.length() < 5) continue;           // muy cortas
        if (stopwords.contains(w)) continue;    // palabras vacías
        if (w.matches("^[0-9]+$")) continue;    // solo números

        frecuencia.put(w, frecuencia.getOrDefault(w, 0) + 1);
    }

    if (frecuencia.isEmpty()) return preguntas;

    // 4. Seleccionar las palabras más frecuentes como "conceptos clave"
    List<Map.Entry<String,Integer>> listaFreq = new ArrayList<>(frecuencia.entrySet());
    listaFreq.sort((a,b) -> Integer.compare(b.getValue(), a.getValue())); // desc

    // Tomamos las top 30 como candidatos
    List<String> conceptosClave = new ArrayList<>();
    int limiteConceptos = Math.min(30, listaFreq.size());
    for (int i = 0; i < limiteConceptos; i++) {
        conceptosClave.add(listaFreq.get(i).getKey()); // en minúscula
    }

    // 5. Patrón para años (por si queremos hacer huecos de fechas)
    Pattern fechaPattern = Pattern.compile("\\b(1[0-9]{3}|20[0-9]{2})\\b");

    // Para evitar repetir siempre el mismo concepto
    Set<String> usados = new HashSet<>();

    // 6. Recorrer oraciones y crear preguntas
    for (String oracion : oraciones) {

        String oracionTrim = oracion.trim();
        if (oracionTrim.length() < 60) continue; // ignorar oraciones muy cortas

        String oracionLower = oracionTrim.toLowerCase();

        String objetivo = null;      // la palabra que vamos a ocultar
        String matchedReal = null;   // la forma exacta como aparece en la oración

        // 6.1 Intentar usar un concepto clave del texto
        for (String concepto : conceptosClave) {

            // Si ya lo usamos muchas veces, mejor pasar
            if (usados.contains(concepto)) continue;

            // Buscamos el concepto como palabra completa (no parte de otra)
            Pattern p = Pattern.compile("\\b" + Pattern.quote(concepto) + "\\b",
                    Pattern.CASE_INSENSITIVE | Pattern.UNICODE_CASE);
            Matcher m = p.matcher(oracionTrim);

            if (m.find()) {
                objetivo = concepto;           // en minúscula
                matchedReal = m.group();      // con mayúsculas/original
                break;
            }
        }

        // 6.2 Si no hay concepto clave, intentamos con fechas como fallback
        if (objetivo == null) {
            Matcher mFecha = fechaPattern.matcher(oracionTrim);
            if (mFecha.find()) {
                objetivo = mFecha.group();
                matchedReal = objetivo;
            }
        }

        // Si no encontramos nada para ocultar, saltamos
        if (objetivo == null || matchedReal == null) continue;

        // 6.3 Construir la oración con hueco
        // Reemplazamos SOLO la primera aparición real por "_____"
        String oracionConHueco = oracionTrim.replaceFirst(
                Pattern.quote(matchedReal),
                "_____"
        );

        String preguntaTexto =
                "Complete la frase según el documento:\n\n\"" +
                        oracionConHueco + "\"";

        // 6.4 Armar el mapa de la pregunta
        Map<String, Object> p = new HashMap<>();
        p.put("tipo", "completar");                   // por si luego quieres mezclar tipos
        p.put("pregunta", preguntaTexto);
        p.put("respuesta_correcta", matchedReal.trim()); // con mayúsculas originales
        p.put("oracion_original", oracionTrim);

        preguntas.add(p);
        usados.add(objetivo);

        // Limitar a 10 preguntas
        if (preguntas.size() == 10) break;
    }

    return preguntas;
}



}
